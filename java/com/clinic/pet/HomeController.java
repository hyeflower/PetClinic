...
//		model.addAttribute("boardList", boardList);
	...
	public String petboardlist(HttpServletRequest request, Model model) {
...
//		model.addAttribute("boardList", boardList);
		...
		if (pageDataCount == null) {
			pageDataCount = "5";// 값이 넘어오지 않았을때 예외처리
		}
		model.addAttribute("boardList", petdao.vetselectAllBoards(page, pageDataCount));
		noticePageDTO vetboardPage = new noticePageDTO();
		int totalDataCount = petdao.bookingdataCount();
		vetboardPage.makePage(Integer.parseInt(page), Integer.parseInt(pageDataCount), totalDataCount);
		model.addAttribute("vetboardPage", vetboardPage);
...
//			model.addAttribute("message", "로그인 하였습니다.");
//		} else if (result == 0) {
...
//		out.println("<script>alert('로그인 하였습니다'); opener.location.reload(); window.close();</script>");
//		out.flush();
...
//			model.addAttribute("message", "회원 가입에 성공했습니다.");
//		} else {
...
//		PrintWriter out = response.getWriter();
//
//		out.println("<script>alert('회원가입이 완료 되었습니다'); window.close();</script>");
//
//		out.flush();
...
//		model.addAttribute("result", result);
	...
	private static final String UPLOAD_PATH_TEST = "D:\\springSTS2020\\Workspace\\PetClinic\\src\\main\\webapp\\resources\\image\\upload";
...
//		UUID uuid = UUID.randomUUID();
		...
		File saveFile = new File(UPLOAD_PATH_TEST, saveName); // 저장할 폴더 이름, 저장할 파일 이름
...
//		PetDao petdao = sqlSession.getMapper(PetDao.class);
//		petdao.insertslideimage(saveName);
//		petdao.selectallimagelist(savaName);
//		model.addAttribute("list",petdao.selectallimagelist(savaName));
		return saveName;
	} // end saveFile(
